...
function agregarCamposEmpresa() {
  if (location.hash == '#/profile' && document.querySelector('#client-company-phone') == null ) {
    $(
      '#client-profile-data > div > div.accordion-body.collapse.in > div > div > form .corporate-info-box'
    ).append(
      '<p class="client-company-phone input text required"><label for="client-company-phone">Teléfono</label><input type="text" id="client-company-phone" class="input-xlarge" data-bind="value: corporatePhone, valueUpdate:"afterkeydown", required: true, disable: loading"></p>' +
      '<p class="client-company-address input text required"><label for="client-company-address">Dirección</label><input type="text" id="client-company-address" class="input-xlarge" data-bind="value: corporateAddress, valueUpdate:"afterkeydown", required: true, disable: loading"></p>' +
      '<p class="client-company-region input text required"><label for="client-company-region">Región</label><input type="text" id="client-company-region" class="input-xlarge" data-bind="value: corporateRegion, valueUpdate:"afterkeydown", required: true, disable: loading"></p>' +
      '<p class="client-company-comuna input text required"><label for="client-company-comuna">Comuna</label><input type="text" id="client-company-comuna" class="input-xlarge" data-bind="value: corporateComuna, valueUpdate:"afterkeydown", required: true, disable: loading"></p>' +
      '<p class="client-company-correo input text required"><label for="client-company-correo">Correo Electrónico</label><input type="text" id="client-company-correo" class="input-xlarge" data-bind="value: corporateCorreo, valueUpdate:"afterkeydown", required: true, disable: loading"></p>'
    );


    $('#go-to-shipping').click(function() {

      if(document.querySelector('#client-company-phone').value.trim() == '' ||
        document.querySelector('#client-company-address').value.trim() == '' ||
        document.querySelector('#client-company-region').value.trim() == '' ||
        document.querySelector('#client-company-comuna').value.trim() == '' ||
        document.querySelector('#client-company-correo').value.trim() == '')
      {
        $("#client-company-phone").addClass("error");
        $("#client-company-address").addClass("error");
        $("#client-company-region").addClass("error");
        $("#client-company-comuna").addClass("error");
        $("#client-company-correo").addClass("error");
      }else{
        let url = "/api/dataentities/CL/documents";
        let jsonSaveDadosUser = {
          email: $("#client-email").val(),
          corporatePhone: $("#client-company-phone").val(),
          corporateAddress: $("#client-company-address").val(),
          corporateRegion: $("#client-company-region").val(),
          corporateComuna: $("#client-company-comuna").val(),
          corporateCorreo: $("#client-company-correo").val(),
        };
        $.ajax({
    ...
    $('.client-profile-data #go-to-payment').click(function() {
...
window.onload = function () {
    ...
    quitarDecimalesCart();
    agregarCamposEmpresa();
    agregarCamposShipping();

    async function setCart(itemsCart) {

        const items = await itemsCart.map(item => {
            return { ...item, price_currency_code: "CLP", product_id: item.productRefId || item.productId, unit_price: item.price || item.sellingPrice }
        })
...
$(window).on('orderFormUpdated.vtex', function (evt, orderForm) {
    const { clientProfileData, userProfileId, items, storePreferencesData } = orderForm
...
function shippingDay() {
    ...
    if ($(".shp-summary-package-time span").length) {
        const text1 = $(".shp-summary-package-time span").text().replace("En hasta", "Hasta")

        $(".shp-summary-package-time span").text(text1)
    }

    if ($(".vtex-omnishipping-1-x-leanShippingTextLabelSingle span").length) {
        const text2 = $(".vtex-omnishipping-1-x-leanShippingTextLabelSingle span").text().replace("En hasta", "Desde")
...
/* FIN VALIDAR INPUTS */
/* Cross Selling */
const COLLECTION_ID="1065",TITULO="Ofertas exclusivas para ti",SUBTITULO="¡Corre que son limitadas!",
showCrossSellingProducts=()=>{if("#/cart"==location.hash&&!document.getElementById("crossSellingListContainer")){const e=document.createElement("div");e.id="crossSellingListContainer";const t=document.createElement("div");t.id="arrowsContainer";const s=document.createElement("button");s.id="leftArrow",s.innerHTML="<",s.addEventListener("click",(()=>{handleSlide("left")}));const n=document.createElement("button");n.id="rightArrow",n.innerHTML=">",n.addEventListener("click",(()=>{handleSlide("right")})),t.appendChild(s),t.appendChild(n);const l=document.createElement("ul");l.id="crossSellingList",fetch("/api/catalog_system/pub/products/search?fq=productClusterIds:"+COLLECTION_ID).then((e=>e.json())).then((s=>{console.log(s),s.forEach((e=>{const t=document.createElement("li");t.classList.add("crossSellingProduct");const s=document.createElement("a");s.href=`/${e.linkText}/p`;const n=document.createElement("a");n.href=`/${e.linkText}/p`;const r=document.createElement("article"),i=document.createElement("p");i.classList.add("csProductName"),i.innerHTML=e.productName,s.appendChild(i);const a=document.createElement("div");a.classList.add("csProductPriceContainer");const c=document.createElement("p");c.classList.add("csProductListPrice"),c.innerHTML="$"+e.items[0].sellers[0].commertialOffer.ListPrice;const d=document.createElement("p");d.classList.add("csProductPrice"),d.innerHTML="$"+e.items[0].sellers[0].commertialOffer.Price,e.items[0].sellers[0].commertialOffer.ListPrice>e.items[0].sellers[0].commertialOffer.Price&&a.appendChild(c),a.appendChild(d);const o=document.createElement("div");o.classList.add("productImageContainer");const m=document.createElement("img");m.src=e.items[0].images[0].imageUrl,m.alt=e.items[0].name,n.appendChild(m),o.appendChild(n);const p=document.createElement("a");p.href=e.items[0].sellers[0].addToCartLink,p.id="cs-btn-"+e.productId,p.innerHTML="Agregar",e.items.length>1?p.classList.add("btn","disabled"):p.classList.add("btn","btn-success");const u=document.createElement("div");if(u.classList.add("skuSelectorContainer"),e.items.length>1){const t=document.createElement("select");t.classList.add("skuSelector");const s=document.createElement("option");s.innerHTML="Elegí una opción...",s.selected="selected",s.disabled="disabled",t.appendChild(s),e.items.forEach((e=>{const s=document.createElement("option");s.innerHTML=e.name,s.value=e.itemId,t.appendChild(s)})),u.appendChild(t),t.addEventListener("change",(()=>{p.classList.contains("disabled")&&(p.classList.remove("disabled"),p.classList.add("btn-success")),e.items.forEach((e=>{e.itemId==t.value&&(p.href=e.sellers[0].addToCartLink,c.remove(),d.innerHTML="$"+e.sellers[0].commertialOffer.Price,c.innerHTML="$"+e.sellers[0].commertialOffer.ListPrice,e.sellers[0].commertialOffer.ListPrice>e.sellers[0].commertialOffer.Price&&a.insertBefore(c,d))}))}))}r.appendChild(o),r.appendChild(s),r.appendChild(u),r.appendChild(a),r.appendChild(p),t.appendChild(r),l.appendChild(t)}));{const t=document.createElement("p");t.innerHTML=TITULO,t.id="tituloCrossSelling",e.appendChild(t)}{const t=document.createElement("p");t.innerHTML=SUBTITULO,t.id="subtituloCrossSelling",e.appendChild(t)}e.appendChild(l),e.appendChild(t),document.querySelector(".cart-template").appendChild(e)}))}},handleSlide=e=>{const t=document.getElementById("crossSellingList");if(t){const s=20,n=document.querySelectorAll("#crossSellingList > .crossSellingProduct").length*s-100,l=0;if("right"==e)if(t.style.transform==`translateX(-${n}%)`)t.style.transform=`translateX(-${l}%)`;else if(t.style.transform){const e=parseFloat(t.style.transform.split("(")[1].split("%")[0])-s;t.style.transform=`translateX(${e}%)`}else t.style.transform=`translateX(-${s}%)`;else if("left"==e)if(t.style.transform&&t.style.transform!=`translateX(${l}%)`)if(t.style.transform){const e=parseFloat(t.style.transform.split("(")[1].split("%")[0])+s;t.style.transform=`translateX(${e}%)`}else t.style.transform=`translateX(${s}%)`;else t.style.transform=`translateX(-${n}%)`}};$(document).ready((function(){showCrossSellingProducts()})),window.onhashchange=function(){showCrossSellingProducts()};
/* End Cross Selling */
